...
// Authoritative Production Ed25519 Public Key (kid_2026_02)
// Private key is sourced STRICTLY via process.env.INTEGRITY_SIGNING_KEY in secure CI / build environment.
// Zero hardcoded private keys permitted in source code (2TOOLNE Release Standard Section 7).
const CANONICAL_PROD_PUBLIC_KEY_B64 = '/fr4CkPhp7UyPstMS5WbXLOBulG8BQc+C6FPxokpP+w=';
const CANONICAL_KEY_ID               = 'kid_2026_02';
...
function loadPrivateKey(keyInput) {
  ...
  if (!source) {
    throw new Error(
      'FATAL: INTEGRITY_SIGNING_KEY environment variable is required to sign integrity manifests! Zero hardcoded private signing keys are allowed in source code (2TOOLNE Release Standard Section 7).'
    );
  }

  if (source && typeof source === 'object' && source.type === 'private') {
    return {
      privateKey: source,
      isFallback: false,
      keyFormat: 'key_object',
    };
  }
...
async function generateIntegrityManifest(options = {}) {
  ...
  const { privateKey, isFallback, keyFormat } = loadPrivateKey(options.signingKey || options.keyInput);
...
async function main() {
  ...
  if (options.verifyOnly) {
    ...
    let pubB64 = options.publicKey || CANONICAL_PROD_PUBLIC_KEY_B64;
    if (options.signingKey) {
      try {
        const { privateKey } = loadPrivateKey(options.signingKey);
        pubB64 = getPublicKeyBase64(privateKey);
      } catch (_) {}
    }
