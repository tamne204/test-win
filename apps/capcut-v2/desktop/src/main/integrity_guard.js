...
// 2. Canonical Public Verification Keys (Key Rotation supported)
const TRUSTED_PUBLIC_KEYS = [
  // Canonical Production Ed25519 Public Key (SPKI Base64)
  'MCowBQYDK2VwAyEA+KnGR3tLyy0jCKMkFuCgh39QSVyP4NNi2R4EspaIkZE=',
  // Raw 32-byte Ed25519 Public Key (Base64)
  '+KnGR3tLyy0jCKMkFuCgh39QSVyP4NNi2R4EspaIkZE=',
];
...
const DEFAULT_BOOTSTRAP_TOKEN_TTL_MS = 60000; // 60 seconds
 ...
 */
function verifyManifest(manifest, publicKey, baseDir = '') {
  ...
  if (baseDir) {
    ...
    for (const [relPath, expectedHash] of Object.entries(manifestPayload.files || {})) {
      ...
      if (!fsOriginal.existsSync(fullPath)) {
        const altCandidates = [
          path.join(baseDir, '..', normalizedRel),
...
async function verifyRuntimeIntegrity(options = {}) {
  ...
  const trustedKeys = options.trustedPublicKeys || TRUSTED_PUBLIC_KEYS;
  ...
  for (const [relPath, expectedHash] of fileEntries) {
    ...
    if (!fsOriginal.existsSync(fullPath)) {
      // Check fallback paths (e.g. parent directory or staging customer package for app.asar)
      const candidates = [
        path.join(baseDir, '..', normalizedRel),
        path.join(baseDir, '..', 'dist', 'win-unpacked', normalizedRel),
        path.join(REPO_ROOT, 'apps', 'capcut-v2', 'desktop', 'dist', 'win-unpacked', normalizedRel),
