...
class AutoUpdateManager extends EventEmitter {
  constructor(options = {}) {
    ...
    this.state = 'IDLE';
    this.currentVersion = options.currentVersion || (app && typeof app.getVersion === 'function' ? app.getVersion() : '2.0.5');
  ...
  async _verifyStagedTree(stagedExtractDir) {
    ...
    // Verify native launcher execution
    console.log(`[AutoUpdateManager] Invoking native root verifier against staged resources:`);
    console.log(`  Launcher : ${launcherCandidate}`);
    console.log(`  Resources: ${resourcesCandidate}`);

    try {
      // Execute the native verifier in verify-only mode
      // On macOS/Linux, ensure executable permissions
      if (!isWin) {
        try { fs.chmodSync(launcherCandidate, 0o755); } catch (_) {}
      }

      // If running on host matching the binary platform, execute verify-only
      ...
      if (!canExecuteLocally && isLauncherExe && isHostMac) {
        // Use local macOS compiled native verifier to validate staged resources directory
      ...
      if (canExecuteLocally || verifierBinaryToRun !== launcherCandidate) {
        ...
        // Cross-platform staging verification (e.g. dev/CI auditing without Wine)
