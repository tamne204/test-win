...
def test_update_integrity_verification():
    with tempfile.NamedTemporaryFile(delete=False) as tf:
        tf.write(b"SlideshowBuilder Binary Content v2.2.3.18")
        tf_path = tf.name
    ...
    try:
        ...
        payload = b'{"version":"2.2.3.18","hash":"' + real_hash.encode() + b'"}'
        ...
        assert verify_package_signature(b'{"version":"2.2.3.18_tampered"}', valid_sig) is False
    finally:
